...
pub fn native_typeof(args: Vec<Value>) -> Result<Value, String> {
    match &args[0] {
        ...
        Value::Function { name: _, body: _, closure: _, params: _ } => Ok(Value::String("fun".to_string())),
        Value::Boolean(_) => Ok(Value::String("boolean".to_string())),
        Value::Null => Ok(Value::String("null".to_string())),
        Value::NativeFunction { name: _, arity: _, fun: _ } => Ok(Value::String("native fun".to_string())),
    }
}
    ...
    match &args[0] {
        Value::String(str) => {
            match str.parse::<f64>() {
                ...
                Err(e) => Err(format!("number(): failed to parse number from string '{}': {}", str, e.to_string()))
            }
        }

        Value::Boolean(b) => {
            if *b { Ok(Value::Number(1.0)) } else { Ok(Value::Number(0.0)) }
        }

        _ => Err(format!("number(): type '{}' cannot be converted to number.", args[0]))
    }
}